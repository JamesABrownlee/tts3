...
class HttpTTSProvider:
    ...
    async def synthesize(self, text: str, voice_id: str, *, max_seconds: int) -> GeneratedAudio:
        ...
        try:
            if resolved_voice == "google_translate":
                return await self._synthesize_google(text, resolved_voice)
            return await self._synthesize_with_fallback(text, resolved_voice, max_seconds=max_seconds)
        except Exception:
            self._mark_voice_failure(resolved_voice)
            fallback_voice = self.settings.fallback_voice_id
            if fallback_voice != resolved_voice:
                return await self._synthesize_google(text, "google_translate") if fallback_voice == "google_translate" else await self._synthesize_with_fallback(text, fallback_voice, max_seconds=max_seconds)
            raise
    ...
    async def _synthesize_with_fallback(self, text: str, voice_id: str, *, max_seconds: int) -> GeneratedAudio:
        if self._tiktok_breaker.is_open():
            return await self._synthesize_google(text, "google_translate")
        for attempt in range(3):
            try:
                return await self._synthesize_tiktok(text, voice_id)
    ...
    async def _synthesize_tiktok(self, text: str, voice_id: str) -> GeneratedAudio:
    ...
    async def _synthesize_google(self, text: str, voice_id: str) -> GeneratedAudio:
